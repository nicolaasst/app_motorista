...
const baselineDir = process.env.VISUAL_BASELINE_DIR || path.resolve(__dirname, '../tests/visual/baseline');
const currentDir = process.env.VISUAL_CURRENT_DIR || path.resolve(__dirname, '../tests/visual/current');
...
async function main() {
  ...
  for (const name of baselineFiles) {
    ...
    if (baseline.width !== current.width || baseline.height !== current.height) {
      results.push({ screen: name, status: 'size-mismatch', diffRatio: null, baseline: `${baseline.width}x${baseline.height}`, current: `${current.width}x${current.height}` });
      continue;
    }

    const { width, height } = baseline;
    const diff = new PNG({ width, height });
    const mismatchedPixels = pixelmatch(baseline.data, current.data, diff.data, width, height, { threshold: 0.1 });
    ...
    results.push({ screen: name, status: pass ? 'pass' : 'fail', diffRatio: Number(diffRatio.toFixed(4)) });
  }
